...
import { saveImage, getImageFullPath } from '../services/imageStorage.js';
...
router.post('/generate-image', async (req: Request, res: Response) => {
  try {
    ...
    console.log('[AI Route] referenceCharIds:', referenceCharIds);
    if (referenceCharIds && referenceCharIds.length > 0) {
      ...
      for (const charId of referenceCharIds) {
        ...
        if (row && row.avatar_path) {
          try {
            const avatarFullPath = getImageFullPath(row.avatar_path);
            console.log('[AI Route] Loading avatar from:', avatarFullPath);
            const avatar = readImageAsBase64(avatarFullPath);
    ...
    // Helper: parse image reference (data URI or server path)
    const parseImageRef = (img: any): { data: string; mimeType: string } | null => {
      if (typeof img === 'string') {
        if (img.startsWith('/api/images/')) {
          try {
            const relativePath = img.replace('/api/images/', '');
            return readImageAsBase64(getImageFullPath(relativePath));
          } catch (err: any) {
            console.warn('[AI Route] Failed to read image from disk:', err.message);
            return null;
          }
        } else {
    ...
    if (sceneReferenceImages && sceneReferenceImages.length > 0) {
      for (const img of sceneReferenceImages) {
        const parsed = parseImageRef(img);
        if (parsed) parsedSceneRefs.push(parsed);
      }
    }

    // v1.9: Parse characterSnapshots (角色快照)
    const parsedSnapshots: Array<{ data: string; mimeType: string }> = [];
    if (characterSnapshots && characterSnapshots.length > 0) {
      for (const img of characterSnapshots) {
        const parsed = parseImageRef(img);
