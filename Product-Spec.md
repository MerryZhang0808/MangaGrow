...
| 导出文件 | 用户本地磁盘（下载） | PNG/JPG 格式图片 |

**注意**：数据存储在本地运行的后端中，清除浏览器缓存不会丢失数据。数据库表已预留 `user_id` 字段，为第二期多用户系统做准备。
