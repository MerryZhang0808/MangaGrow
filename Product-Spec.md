...
这是一个帮助家长将孩子成长瞬间转化为漫画故事的 AI 应用。家长经常因为来不及拿手机而错过记录孩子的成长瞬间，或者拍了很多照片视频却不知道如何整理。本产品支持家长通过文字描述、语音记录、照片上传、视频上传等多种方式输入故事，AI 自动生成漫画分镜脚本和配图，生成温馨可爱的漫画故事，既保护隐私（不用发社交平台），又方便保存和分享。适用于家长记录孩子日常趣事、成长里程碑、家庭温馨时刻。

## 功能需求

**第一期核心功能：**
- 多种输入方式：支持文字输入、语音录制（按住说话）、照片上传、视频上传，可混合输入
- 视频转漫画：支持上传视频（MP4/MOV，≤3分钟，≤500MB），上传后立即开始 AI 分析（不等用户点生成），AI 同时完成视频内容理解（生成文字描述）和关键帧提取（2-4张）；分析完成后关键帧以缩略图展示在输入区，用户可删除不想要的帧；关键帧数量决定分镜数量（1:1 对应）；视频和照片可混合上传，混合时关键帧决定分镜骨架，照片作为额外视觉参考不增加分镜数；每次最多上传 1 段视频；超过 3 分钟直接拒绝并提示
...
- **素材上传区**：
  - 位置：文字输入框下方
  - 标题：「参考素材」（Reference Media）
  - 并排两个上传按钮：「添加照片」按钮（图片图标）+ 「添加视频」按钮（视频图标）
  - 已上传照片显示为缩略图卡片（80x80px），可删除
  - 已上传视频显示为视频缩略图卡片（80x80px）+ 时长标签 + 分析状态：
    - 上传中：进度条
    - 分析中：「AI 分析中...」+ 动画
    - 分析完成：显示提取的关键帧缩略图（与照片缩略图并排展示，带"关键帧"标签），用户可删除单个关键帧
    - 失败：错误提示 + 重试按钮
  - 视频限制：MP4/MOV 格式，≤3 分钟，≤500MB，最多 1 段；超出限制时前端直接拒绝并提示原因
...
1. 打开应用，看到欢迎界面和初始提示
2. 在中间输入区输入文字描述或按住录音讲述故事，或上传照片/视频（可混合）
   - 上传视频后，AI 立即开始分析（上传进度 → 分析中 → 展示关键帧缩略图）
   - 用户可在等待视频分析的同时继续写文字、选风格
...
| POST | `/api/ai/analyze-video` | 视频内容分析（理解内容 + 提取关键帧） |
...
5. **视频变漫画**
   - 爸爸赵先生拍了一段 30 秒的视频：1岁半的儿子扶着沙发站起来，踉跄走了三步，然后跌坐在地毯上笑了。他打开应用，直接上传这段视频，一个字都没写。AI 自动分析视频内容，提取了 3 个关键帧（站起来、走路、跌坐笑），生成文字描述"小宇扶着沙发勇敢站起来，迈出三步后跌坐在软软的地毯上，咯咯笑个不停"。AI 据此生成 3 张漫画分镜，完美还原了这个珍贵的"第一次走路"瞬间。赵先生觉得比视频本身更有故事感。
...
[技能]
    ...
    - **视频分析**：理解视频内容生成文字描述，同时提取关键帧作为漫画分镜参考
...
[AI 功能]
    ...
    **提示词**：
    ...
    ### 视频内容分析
    **用途**：分析用户上传的视频，理解视频内容并提取关键帧，为漫画故事生成提供素材
    **输入**：用户上传的视频文件（MP4/MOV，≤3分钟）
    **输出**：JSON 格式的分析结果（文字描述 + 关键帧图片）
    ...
    **提示词**：
    ...
    任务二：关键帧提取
    从视频中选取 2-4 个最能代表故事发展的关键时刻，要求：
    - 每个关键帧代表故事的一个阶段（起承转合）
    - 关键帧之间要有叙事推进，不能是重复的画面
    - 优先选择人物表情清晰、动作有代表性的画面
    - 返回每个关键帧的时间戳

    输出 JSON 格式：
    {
      "description": "视频内容的完整文字描述（100-200字）",
      "keyFrames": [
        { "timestamp": "00:03", "description": "这一帧的画面描述" },
        { "timestamp": "00:12", "description": "这一帧的画面描述" }
      ]
    }
    ```
...
[工作流程]
    ...
    [输入收集阶段]
        ...
        第一步：判断输入类型
            ...
            - 用户上传视频 → 立即调用 [视频内容分析] → AI 理解视频内容生成文字描述 + 提取关键帧 → 关键帧展示在输入区
            - 可混合输入（文字+照片+视频、语音+视频等任意组合）
...
| **Video Understanding** | 用于分析用户上传的视频，理解视频内容生成文字描述，同时提取关键帧作为漫画分镜的参考图 |
