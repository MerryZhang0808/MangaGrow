...
- 脚本生成：为每个分镜生成适合漫画表现的文字脚本
- 漫画图片生成：根据脚本生成对应的漫画图片，支持 4 种风格（温馨卡通、柔和水彩、简约扁平、手绘涂鸦）
- 脚本编辑与重新生成：用户可修改某个分镜的脚本，单独重新生成对应图片
- 人物库管理：创建人物（名字+照片→AI识别性别/年龄→生成卡通形象→可重新生成），AI 自动识别故事中的人物名字并匹配人物库
- 导出功能：支持 PNG/JPG 格式导出漫画图片，可单张或打包下载；支持网格海报式导出（所有分镜拼合为一张完整海报图）
- 故事标题：AI 根据故事内容自动生成简短标题（如"念念第一次吃饭"），用户可修改
- 自动保存与同步：生成完成后自动保存到历史记录，用户修改分镜后自动同步更新
- 历史记录浏览：左侧导航栏新增历史图标，点击打开历史侧边栏，选中某条记录后在右侧面板呈现（与生成结果展示结构一致）
...
- **纵向布局**：
  - **上方（70%）**：漫画图片区
    ...
    - 鼠标悬停显示操作按钮（右上角）：
      - 「重绘」按钮（Redraw，刷新图标）
      - 「编辑脚本」按钮（Edit Script，编辑图标）
  - **下方（30%）**：脚本文字区
    - 脚本文字（居中显示，可编辑）
    - 点击进入编辑模式，显示文本框和「保存并重绘」「取消」按钮
...
### 左侧折叠侧边栏 - 历史记录（覆盖在输入区上方，与人物库交互一致）
...
- 每条记录卡片包含：
  - 故事标题（AI 生成，如「念念第一次吃饭」）
  - 创建时间（如「2026年2月14日」）
  - 第一张分镜缩略图（圆角矩形，80x60px）
  - 分镜数量标识（如「4 格」）
- 点击卡片 → 在右侧面板加载该故事（与生成结果展示结构完全一致）
- 当前正在查看的记录高亮显示

**空状态**：
- 无历史记录时显示：「还没有创作记录，开始你的第一个故事吧！」
...
4. 点击「生成漫画」按钮
5. AI 分析输入，生成分镜脚本
6. 右侧流式显示分镜卡片（上方图片+下方脚本）
7. 生成完成后：
   - AI 自动生成故事标题（显示在右侧面板顶部，可点击编辑）
   - 故事自动保存到历史记录
   - 可以：
     - 鼠标悬停在分镜卡片上，点击「重绘」按钮重新生成该分镜
     - 点击「编辑脚本」按钮修改脚本，然后「保存并重绘」（修改后自动同步保存）
     - 如果有后续分镜，点击「继续生成后续」
     - 满意后点击右下角「导出」按钮，选择「导出海报」或「导出 ZIP」
...
1. 点击左侧导航栏「历史记录」图标，打开历史侧边栏
2. 按时间倒序查看所有已保存的漫画故事
3. 每条记录显示：标题 + 日期 + 第一张分镜缩略图 + 分镜数量
4. 点击某条记录
5. 右侧面板加载该故事的所有分镜（展示结构与生成结果完全一致）
6. 中间输入区标题和日期同步更新为该故事的标题和创建时间
7. 可以对历史故事执行所有操作：重绘分镜、编辑脚本、导出海报/ZIP
8. 修改后自动同步保存
...
| 漫画历史 | SQLite `stories` + `scenes` 表 + `data/images/scenes/` | 故事元数据存 DB，分镜图片存磁盘 |
| API Key | 服务端 `.env` 文件 | 不暴露给前端，通过后端代理调用 |
| 导出文件 | 用户本地磁盘（下载） | PNG 海报图片 或 ZIP 打包文件 |

**注意**：
- 数据存储在本地运行的后端中，清除浏览器缓存不会丢失数据
- 数据库表已预留 `user_id` 字段，为第二期多用户系统做准备
- 漫画生成完成后自动保存，修改分镜后自动同步，用户无需手动保存
...
[总体规则]
    ...
    - 生成完成后自动保存到历史记录（包含 AI 生成的标题、分镜脚本和图片）
    - 用户修改分镜（编辑脚本、重绘图片、修改标题）后自动同步更新到后端
    - 始终保持温馨、友好的语气
...
[工作流程]
    ...
    [图片生成阶段]
        ...
        第二步：生成完成
            - 调用 [故事标题生成]，生成故事标题，显示在右侧面板顶部
            - 自动保存：调用后端 POST /api/stories，保存故事（标题 + 分镜脚本 + 图片）
            - 显示："✅ 漫画生成完成！"
    ...
    [脚本修改与重新生成阶段]
        ...
        第三步：更新显示并同步保存
            替换该分镜的图片
            自动调用后端 PUT /api/stories/:id 同步更新
    ...
    [历史记录浏览阶段]
        ...
        第一步：打开历史侧边栏
            ...
            - 按时间倒序显示（标题 + 日期 + 缩略图 + 分镜数）

        第二步：选中某条记录
            - 调用后端 GET /api/stories/:id 获取故事详情（含分镜）
            - 右侧面板加载分镜展示（与生成结果展示结构完全一致）
            - 输入区标题和日期同步更新

        第三步：操作历史故事
            - 可执行：重绘分镜、编辑脚本、修改标题、导出
            - 所有修改自动同步保存到后端

        第四步：删除历史
            - 长按或右键卡片，选择「删除」
            - 确认后调用后端 DELETE /api/stories/:id
            - 从列表中移除
