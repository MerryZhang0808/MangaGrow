...
const App: React.FC = () => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      // v1.9: 角色快照累积机制 — 维护每个角色的最佳已生成分镜
      const characterSnapshots: Map<string, string> = new Map(); // charId -> bestSceneUrl
      ...
      const generateOne = async (scene: Scene, sceneIndex: number, sceneRefImages: string[], isPhoto: boolean) => {
        ...
        // v1.9: 收集当前分镜涉及角色的快照 URL
        const snapshotUrls: string[] = [];
        for (const charId of referenceCharIds) {
          const snapshot = characterSnapshots.get(charId);
          if (snapshot) snapshotUrls.push(snapshot);
        }
        ...
        try {
          ...
          // v1.9: 更新涉及角色的快照（当前分镜成为该角色的最佳参考）
          for (const charId of referenceCharIds) {
            characterSnapshots.set(charId, imageUrl);
          }
      ...
      if (hasUserPhotos) {
        ...
        // v1.9: 增强链式策略 — 前一分镜(画风) + 角色快照(角色一致性)
        ...
        for (let i = 0; i < firstBatch.length; i++) {
          ...
          if (result) prevImageUrl = result; // 每张成为下一张的画风参考
        }
      }
