...
import { SceneImageParams } from "../types";
import { getAiClient, withRetry, SAFETY_SETTINGS, IMAGE_MODEL } from "./aiClient";
...
export async function generateSceneImage(params: SceneImageParams): Promise<string> {
  ...
  const executeGeneration = async (hasVisuals: boolean) => {
    ...
    if (hasVisuals) {
      ...
      if (params.sceneReferenceImages.length > 0) {
        const sceneImg = await compressImage(params.sceneReferenceImages[0]);
      ...
      for (const char of activeRefs) {
        ...
        // Reference sheet (if POC-01 passed and available)
        if (char.referenceSheetUrl) {
          const sheetImg = await compressImage(char.referenceSheetUrl);
          const sheetParsed = parseDataUri(sheetImg);
          parts.push({ inlineData: { mimeType: sheetParsed.mimeType, data: sheetParsed.data } });
  ...
  try {
    if (hasAnyRefs) {
      console.log(`[imageService] Generating with visual refs (Scene: ${params.sceneReferenceImages.length}, Chars: ${params.referenceChars.length})`);
      return await executeGeneration(true);
    } else {
      console.log("[imageService] Generating text-only");
      return await executeGeneration(false);
    }
  } catch (e: any) {
    // C14: fallback to text-only on ref failure
    if (hasAnyRefs) {
      console.warn("[imageService] Visual ref generation failed, falling back to text-only:", e);
      return await executeGeneration(false);
    }
