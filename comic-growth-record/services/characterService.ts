...
import { getAiClient, withRetry, SAFETY_SETTINGS, TEXT_MODEL, IMAGE_MODEL } from "./aiClient";
import { parseDataUri, compressImage } from "../utils/imageUtils";

// Step 1: Analyze photo and generate detailed visual description
export async function analyzeCharacter(name: string, photoDataUri: string): Promise<string> {
  const ai = getAiClient();
  const compressedUri = await compressImage(photoDataUri);
...
// Step 2: Generate Q-version avatar (2K resolution, with photo reference)
...
// Combined entry: create a full character (analyze → avatar)
export async function createCharacter(
  ...
  try {
    description = await analyzeCharacter(name, photoDataUri);
  } catch (e) {
    console.error("Character analysis failed, using fallback description", e);
    description = `一个可爱的卡通版 ${name}`;
  }

  // Step 2: Generate avatar
  const avatarUrl = await generateAvatar(name, photoDataUri, description);
  ...
  return {
    ...
    avatarUrl,
    description,
    originalPhotoUrls: [photoDataUri],
    createdAt: Date.now()
  };
}

// Filter and return character references relevant to a scene script
export function getCharacterReferences(
  characters: Character[],
  sceneScript: string
): CharacterRef[] {
  // Sort by relevance: characters mentioned in the script come first
  return characters
    .filter(c => c.avatarUrl) // Only characters with avatars
    .sort((a, b) => {
      const aIn = sceneScript.includes(a.name);
      const bIn = sceneScript.includes(b.name);
      if (aIn && !bIn) return -1;
      if (!aIn && bIn) return 1;
      return 0;
    })
