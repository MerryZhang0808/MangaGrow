...
## [v2.0] - 2026-03-06
### 新增（视频转漫画）
- **视频上传输入**：新增视频上传入口（与照片按钮并排），支持 MP4/MOV 格式，≤3 分钟，≤500MB，每次最多 1 段
- **视频内容分析**：上传后立即调用 AI 分析（Video Understanding），同时完成视频内容理解（生成文字描述）和关键帧提取（2-4 张）
- **关键帧展示与管理**：分析完成后关键帧以缩略图展示在输入区（带"关键帧"标签），用户可删除不想要的帧
- **关键帧驱动分镜**：关键帧数量决定分镜数量（1:1 对应）；混合上传时（视频+照片），关键帧决定分镜骨架，照片作为额外视觉参考不增加分镜数
- **新增 AI 功能**：视频内容分析提示词（理解视频内容 + 提取关键帧时间戳和描述）
- **新增 API 端点**：`POST /api/ai/analyze-video`
- **新增 Google AI 能力**：Video Understanding
- **新增应用场景**：视频变漫画（爸爸上传 30 秒走路视频，AI 自动生成 3 张分镜漫画）

### 修改
- **产品说明**：输入方式增加"视频上传"
- **输入区 UI**：「参考照片」改为「参考素材」，并排照片和视频两个上传按钮
- **输入收集工作流**：新增视频输入路径（上传即分析，不等用户点生成）

### 从第二期移入第一期
- **视频直接转漫画**：从"第二期功能规划"移入第一期核心功能

### 修复
- **第二期编号**：修复编号跳跃（1→3）和重复（两个 7）问题
