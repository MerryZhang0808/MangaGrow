...
## [v1.9.1] - 2026-03-05
### 修复
- **服装策略检测逻辑**：`detectCharacterStyleMode` 现在只检测用户原始输入，不再检测 AI 生成的脚本
- **原因**：AI 可能在脚本中添加用户未明确指定的服装描述（如「穿着小裙子」），导致误判为 `head-only` 模式，从而丢失人物库 Q 版形象的服装一致性

### 解决的问题
- **人物库形象不一致**：当用户未指定服装时，应使用 `full` 模式（Q 版完整形象），但原逻辑检测 AI 脚本后误判为 `head-only`，导致生成人物与人物库形象不匹配

---

## [v1.9] - 2026-03-05
### 新增
- **角色快照累积机制**：分镜生成时维护每个角色的最佳已生成分镜快照，后续分镜生成时注入该快照作为角色一致性参考
- **characterSnapshots 参数**：`GenerateImageParams` 和 `SceneGenerationParams` 新增 `characterSnapshots` 字段
- **服装策略前置判断**：`storyPipeline.ts` 新增 `detectCharacterStyleMode` 函数，根据故事描述是否包含服装关键词判断策略
- **characterStyleMode 参数**：`'full'`（用Q版完整形象）或 `'head-only'`（只用Q版面部，服装用描述）

### 修改
- **分镜间一致性策略**：从「仅前一分镜链式传递」升级为「前一分镜(画风) + 角色快照(角色一致性)」双重参考
- **提示词格式优化**：从长文本（~1500字）改为紧凑 YAML 格式（~600字），6层结构保持不变
- **服装规则动态化**：根据 `characterStyleMode` 动态生成服装规则，确保同一故事内策略统一
- **App.tsx 生成逻辑**：`handleGenerate` 中维护 `characterSnapshots` 映射，传递 `characterStyleMode` 给 API
- **imageGenerator.ts**：提示词改为 YAML 格式，参考图标签简化，新增 `characterStyleMode` 支持
- **storyPipeline.ts**：`StoryOutput` 新增 `characterStyleMode` 字段，管线结束时判断策略
- **agent-flow-diagram.html**：同步更新分镜生成流程图（Prompt 结构变化、新增字段）

### 解决的问题
- **角色外观漂移**：同一角色在不同分镜中可能因独立生成导致外观不一致，现在通过角色快照累积机制增强一致性
- **提示词过长**：原提示词约1500字导致模型关注分散，现精简为约600字 YAML 格式
- **服装策略不统一**：原提示词让模型独立判断服装来源，可能导致同一故事内策略不一致，现由管线前置判断并统一注入

### 测试结构重组
- **新增** `tests/Test-Cases.md`：统一测试用例库（功能/UI/架构约束）
- **新增** `tests/Test-Records/`：测试执行记录归档目录
- **移动** `QA-Report.md` → `tests/QA-Report.md`：自动化检查报告
