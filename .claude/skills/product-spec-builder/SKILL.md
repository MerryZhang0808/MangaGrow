...
[工作流程（0-1模式）]
    ...
    [文档生成阶段]
        ...
        完成后：
            ...
            ### 方式 A：先做架构设计（推荐）
            输入 /arch 进行架构设计，为每个功能确定技术方案和质量标准。
            架构设计会产出 Architecture.md 和 Tasks.md，确保代码实现有据可依。

            ### 方式 B：直接去 AI Studio 生成代码
            1. 打开 https://aistudio.google.com/app/builder
            2. 点击「Build an app」
            3. 将 Product Spec 内容复制到项目描述中
            4. 根据「AI 能力配置」表格勾选对应能力
            5. 预览测试后下载代码
            6. 解压到当前项目文件夹
            7. 输入 /arch 补充架构设计，或 /run 直接运行

            💡 推荐先走方式 A，确保后续开发有明确的技术方案和质量标准。"
    ...
    **核心原则**：无缝衔接，不打断用户工作流。不需要开场白，直接接住用户的需求往下问。
    ...
    [文档更新阶段]
        ...
        完成后：
            ...
            下一步：输入 /arch 更新架构设计，确保技术方案与最新需求一致。"
